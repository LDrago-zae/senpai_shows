...
class AniListApiService {
  ...
  Future<List<Anime>> fetchPopularAnime({
    int page = 1,
    int perPage = 10,
  }) async {
    ...
    final Map<String, dynamic> variables = {'page': page, 'perPage': perPage};
  ...
  Future<List<Anime>> fetchRecommendedAnime({
    int page = 1,
    int perPage = 10,
  }) async {
    const String query = r'''
  ...
  ''';
    ...
    final variables = {'page': page, 'perPage': perPage, 'genre': genre};
      ...
      if (response.statusCode == 200) {
        ...
        return mediaList.map((item) {
          return Anime(
            ...
            rank: 0,
            starring:
                item['studios']?['nodes']?.isNotEmpty == true
        ...
        throw Exception(
          'Failed to fetch $genre anime from AniList: ${response.statusCode}',
        );
      }
    } catch (e) {
      debugPrint('Error fetching $genre anime from AniList: $e');
  ...
  ''';
    ...
    final variables = {'page': page, 'perPage': perPage, 'search': query};
