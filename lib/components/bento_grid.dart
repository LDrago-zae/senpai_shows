...
class _BentoGridState extends State<BentoGrid> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _scaleAnimations = _controllers.map((controller) => Tween<double>(begin: 0.8, end: 1.0).animate(
      CurvedAnimation(parent: controller, curve: Curves.easeOutBack),
    )).toList();
    _opacityAnimations = _controllers.map((controller) => Tween<double>(begin: 0.0, end: 1.0).animate(
        ...
        Row(
          children: [
            Expanded(
              child: SizedBox(
                height: 160,
                child: _buildAnimeCard(context, widget.animeList[5 % widget.animeList.length], 5),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: SizedBox(
                height: 160,
                child: _buildAnimeCard(context, widget.animeList[6 % widget.animeList.length], 6),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: SizedBox(
                height: 160,
                child: _buildAnimeCard(context, widget.animeList[7 % widget.animeList.length], 7),
              ),
            ),
          ],
        ),
